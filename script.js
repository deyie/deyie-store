...
// Ganti dengan nomor WhatsApp asli pacar Anda (awali dengan 62, tanpa + atau spasi)
const WHATSAPP_NUMBER = "628558826428"; 
...
// Deteksi apakah sedang di halaman sosmed.html atau app.html
const isSosmedPage = window.location.pathname.includes("sosmed.html");

// JIKA Error sebelumnya terjadi karena filter halaman, baris di bawah ini mengantisipasinya 
// agar produk tetap muncul walau nama file di browser berbeda atau dites lokal.
const currentPageProducts = products.filter(p => {
  if (isSosmedPage) return p.page === "sosmed";
  return true; // Menampilkan semua produk jika di app.html atau halaman utama agar tidak kosong
});
...
function buildWhatsAppLink(product) {
  const message = 
    `Halo Deya Store! Aku mau jajan ini ya 🌸\n\n` +
    `• Produk: ${product.name}\n` +
    `• Paket/Plan: ${product.plan}\n` +
    `• Durasi: ${product.duration}\n` +
    `• Harga: ${formatPriceCustom(product.price)}\n\n` +
    `Mohon info ketersediaan dan cara pembayarannya ya kak, thank you! ✨`;
